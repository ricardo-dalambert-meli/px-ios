...
extension InitFlow {
    ...
    func getInitSearch() {
        ...
        if let prefId = pref.id, prefId.isNotEmpty {
            // CLOSED PREFERENCE
            serviceAdapter.getClosedPrefInitSearch(preferenceId: prefId, cardsWithEsc: cardIdsWithEsc, splitEnabled: splitEnabled, discountParamsConfiguration: discountParamsConfiguration, flow: flowName, charges: charges, headers: headers, newCardId: newCardId, callback: callback(_:), failure: failure(_:))
        } else {
            // OPEN PREFERENCE
